...
// =================================================
// PIN CONNECTION (Arduino UNO)
// =================================================

// 3 PZEM common communication bus
// Arduino D2 = RX (SoftwareSerial)
// Arduino D3 = TX (SoftwareSerial)
...
// Buffer sizes (reduced for Arduino UNO limited RAM: 2KB)
const int SMS_BUFFER_SIZE = 128;  // Reduced from 256 for UNO
const int AT_RESPONSE_SIZE = 256; // Reduced from 512 for UNO
...
void setup()
{
  ...
  Serial.println("\n================================");
  Serial.println("3 Phase Motor Controller v2.0");
  ...
  // Resets Arduino if no wdt_reset() call for 4 seconds
  wdt_enable(WDTO_4S);
  ...
  if (!pzemsInitialized) {
    Serial.println("[WARNING] PZEM init may have failed. Check connections.");
  } else {
    Serial.println("[OK] PZEMs initialized successfully.");
  }
  ...
  if (!gsmInitialized) {
    Serial.println("[ERROR] GSM init failed. Check SIM and power.");
  } else {
    Serial.println("[OK] GSM module initialized successfully.");
  }

  Serial.println("\n[SYSTEM] Ready. Motor OFF by default.\n");
...
bool initializeGSM()
{
  ...
  if (sendATCommand("AT", "OK", 2000)) {
    Serial.println("[GSM] AT command OK");
    atOk = true;
  }
  ...
  if (sendATCommand("AT+CMGF=1", "OK", 2000)) {
    Serial.println("[GSM] SMS format set");
    cmgfOk = true;
  }
  ...
  if (sendATCommand("AT+CSCS=\"GSM\"", "OK", 2000)) {
    Serial.println("[GSM] Charset set");
    cscsOk = true;
  }
...
void checkSMSCommands()
{
  ...
  // Check for valid SMS commands
  if (sms.indexOf("MOTOR ON") >= 0 || sms.indexOf("ON") >= 0) {
    Serial.println("[SMS CMD] Motor ON");
    motorON();
    delay(1000);
    clearSMSMessage();
  }
  else if (sms.indexOf("MOTOR OFF") >= 0 || sms.indexOf("OFF") >= 0) {
